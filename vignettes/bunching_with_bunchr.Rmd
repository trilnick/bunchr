...
A growing literature in empirical economics is focused around bunching. Loosely
...
Olsen, T., Pistaferri, L. (2009). _"Adjustment Costs, Firm Responses, and Micro
...
The `bunchr` package can be used to visualize bunching and estimate the elasticity
of earnings w.r.t. marginal tax rates, as usually done in the literature cited
above. There is also a function to simulate earnings given certain parameters.
...
potentially set some of the parameters for analysis.
...
```{r first_kink}
kink_est <- bunchr(earning_vec, zstar = 1000, t1 = 0, t2 = 0.1, Tax = 0,
                   cf_start = 10, cf_end = 10,
                   exclude_before = 2, exclude_after = 2, binw = 50)
kink_est$e
```
Note: we modeled the earnings with elasticity of 0.2, but got an estimate of 0.16.
This might be the result of the width of the bins, the width of the excluded area
around the kink, or both. Let's try this with narrower bins. Note, I have to adjust
the other parameters. However, I'll keep the excluded area at the same width.

```{r kink_narrower}
kink_est <- bunchr(earning_vec, zstar = 1000, t1 = 0, t2 = 0.1, Tax = 0,
                   cf_start = 50, cf_end = 50,
                   exclude_before = 10, exclude_after = 10, binw = 10)
kink_est$e
```

That didn't do much to the elasticity parameter, but we have narrower bins. Now,
we narrow the excluded area around the kink point. In fact, since I generated a
"clean" simulation, I can narrow the excluded area to the bunching bin itself,
setting _exclude_before_ and _exclude_after_ to zero. For the demonstrative
purpose of this exercise, I'll keep the excluded area to one bin before and one
bin after the kink bin. I'll also bootstrap the results to get a sense of the
potential distribution of elasticity parameters. Bootstrapping is done by
resampling the population vector and running the analysis.
...
These results are much closer to the original. Just for the purpose of demonstration,
let's run the analysis with the kink bin only as excluded area. I do not include the plotp lot this time.
```{r kink_boot0}
kink_est <- bunchr(earning_vec, zstar = 1000, t1 = 0, t2 = 0.1, Tax = 0,
                   cf_start = 50, cf_end = 50,
                   exclude_before = 0, exclude_after = 0, binw = 10,
...
notch_est <- bunchr(earning_vec, zstar = 1000, t1 = 0.1, t2 = 0.1, Tax = 200,
                   cf_start = 50, cf_end = 50,
...
what visibly seems like the end of the notch. In that case, it might be
useful to change with the exclude_after parameter and see if the plot come out
better.

Another issue is the counter-factual distribution. For larger notches, the
inetrpolation is harder. Remember the interpolation is based on the histogram
...
kink_est <- bunchr(earnings_small, zstar = 1000, t1 = 0, t2 = 0.1, Tax = 0,
                   cf_start = 10, cf_end = 10,
...
kink_est <- bunchr(earnings_large, zstar = 1000, t1 = 0, t2 = 0.1, Tax = 0,
                   cf_start = 50, cf_end = 50,
...
kink_est <- bunchr(earning_vec, zstar = 1000, t1 = 0.1, t2 = 0.1, Tax = 0,
                   cf_start = 50, cf_end = 50,
                   exclude_before = 1, exclude_after = 1, binw = 10,
                   nboots = 100, seed = 123, draw = F)
```
...
But let's see what `bunchr` does when we try to estimate a marginal tax rate
...
What would happen if bunching does occur, but we did not specity the tax rates
correctly when running `bunchr`? Let's create an earnings vector with a tax rate
change from 0% to 30%, but run `bunchr` for a smaller tax change, 0% to 20%. As
the observed bunching is higher than what would be expected for a change from 0%
to 20% with elasticity of 0.2, `bunchr` estimates a higher elasticity of earnings.
...
earning_vec <- sapply(ability_vec, earning_fun, elas = 0.2, t1 = 0, t2 = 0.3,
                      Tax = 0, zstar = 1000)
kink_est <- bunchr(earning_vec, zstar = 1000, t1 = 0, t2 = 0.2, Tax = 0,
                   cf_start = 50, cf_end = 50,
                   exclude_before = 1, exclude_after = 1, binw = 10,
                   nboots = 100, seed = 123, draw = F)
kink_est$results$e

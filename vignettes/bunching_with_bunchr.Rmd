...
(_zstar_ = 1000), at which the marginl tax rate increases from 0% to 10%
(_t1_ = 0, _t2_ = 0.1), the notch height is zero (_Tax_ = 0). After creating the
earning vector, we can view it with `abunch_viewer`.
...
and test for similarity in results. The _Stata_ program was written by Tore Olsen,
and can be found in Raj Chetty's [website](http:/www.rajchetty.com/chettyfiles/bunch_count.zip).
To replicate these results, you will need to download and install the `bunch_count`
function for _Stata_, and of course define the working directories properly.

...
knitr::opts_chunk$set( fig.width = 7, fig.height = 4,
                      warning = TRUE,
                      error = TRUE )
#knitr::opts_knit$set(root.dir = "C:\\Users\\User\\Box Sync\\btl\\bunching\\bunchr\\inst\\compare")
#
```
...
an attenuating effect. We might not be very happy with the bootstrappped 
...
will need to download and install the `bunch_count` package for _STATA_, and 
of course define the working directories properly.

First, I create a dataset with `bunchr`. The _STATA_ package required collapsed,
binned data. I use `bunch_viewer` to create and export that.
...
``` {r chetty3, eval=TRUE}
chetty_res <- read.csv(file="compare_results.csv")
chetty_res <- chetty_res[order(chetty_res$earnings), ]
``` 

Now, we can run `bunchr` on the data. I use the same specification regarding
the bin width, counter-factual area, excluded area, and correcting for shift on
the right side of the notch. The main difference is that, since `bunchr` calculates
...
``` {r chetty4, eval=TRUE}
estim <- bunchr(earning_vec, 1000, 0, 0.2, 0, 10, 10, 1, 1, 50, max_iter = 200, correct = T, select = F, poly_size = 7, draw = F)
